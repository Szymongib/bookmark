use crate::interactive::subcommand::{require_string, ask_for_string};
...
impl AddData {
    pub fn new(name: &str, url: &str, group: &str, tags: &[String]) -> AddData {
        AddData{
            ...
            tags: tags.to_vec().into_iter().filter(|t| !t.is_empty()).collect()
        }
    }
...
pub fn interactive_add(add_data: AddData) -> Result<AddData, Box<dyn std::error::Error>> {
    ...
    let tags: Vec<String> = tags_raw.split(", ").into_iter().map(|f|{f.to_string()}).collect();
